...
// Axios instance
// ---------------------------------------------------------------------------
// withCredentials is false because we are using token auth (Authorization:
// Bearer), not session cookies. Setting it to true cross-domain causes
// browsers to enforce SameSite/CORS cookie rules that block requests.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// No getCsrfToken() call needed — token auth does not use CSRF cookies.
// The login endpoint returns { token, user }; store token in localStorage.
// ---------------------------------------------------------------------------
